...
#define SORT_AND_UNIQ_VECTOR						\
  ...
  namespace
	...
	template< typename T >
	  ...
	  {
	    typedef typename T::COLUMN_INFO_TYPE::BELONG_TABLE_TYPE type;
	  };

	//order by 指定句をカラムが所属するテーブルのベクタ型に変換する
	template<typename First, typename... Rest >
	  struct order_by_specify_2_belong_table_vector
	  {
	  private:
	    //引数ベクタ
  ...
  //order_byカラムが所属するテーブルタプルを取得する
#define FUNCTION_LOCAL_GET_BELONG_TABLE_TUPLE				\
  typename mpl_util::vector_2_tuple<					\
    typename mpl_util::sort_and_uniq_table_vector<				\
    typename work_order_by::order_by_specify_2_belong_table_vector< First ,  Rest... >::type \
    >::type								\
    >::type								\
  
  template<typename First, typename... Rest >
    query_object_order_by< FUNCTION_LOCAL_GET_BELONG_TABLE_TUPLE > order_by(void)
  {
    
    typedef FUNCTION_LOCAL_GET_BELONG_TABLE_TUPLE TABLE_TYPE;
    
    const query_object< TABLE_TYPE > t;
    query_object_order_by< TABLE_TYPE >  ret( t ) ;
    ret.template order_by< First ,  Rest... >();
